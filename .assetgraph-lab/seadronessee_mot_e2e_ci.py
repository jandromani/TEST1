...
import json,pathlib,urllib.request,hashlib,os,math,statistics,shutil
...
def metrics(pred_by_frame,gt_by_frame,frames):
 ...
 if pids and gids:
  ...
  pi={x:i for i,x in enumerate(pids)};gi={x:i for i,x in enumerate(gids)}
...
def acquire_frames(images):
 ...
 rgb=ASSET/'rgb';rgb.mkdir(exist_ok=True)
 rz=RemoteZip(ZIP_URL);names=rz.namelist();bybase=defaultdict(list)
 for n in names:bybase[pathlib.PurePosixPath(n).name].append(n)
 acquired=[]
 for im in images:
  fn=im.get('file_name') or im.get('filename');base=pathlib.PurePosixPath(fn).name if fn else None;cands=[]
  if fn:
   cands=[n for n in names if n.endswith(fn)]
  if not cands and base:cands=bybase.get(base,[])
  if not cands:
   # Last-resort mapping using source folder + original frame number tokens.
   src=im.get('source',{});folder=str(src.get('folder_name',''));frame=str(src.get('frame_no',''))
   cands=[n for n in names if (folder and folder in n and frame and frame in pathlib.PurePosixPath(n).stem)]
  if not cands:raise RuntimeError(f'No ZIP member for image_id={im.get("id")} file={fn} source={im.get("source")}')
  member=sorted(cands,key=len)[0];b=rz.read(member);ext=pathlib.PurePosixPath(member).suffix or '.jpg';p=rgb/f'{im["id"]:06d}{ext}';p.write_bytes(b)
  with Image.open(p) as chk:chk.verify()
  acquired.append({'image_id':im['id'],'member':member,'path':str(p),'sha256':sha_bytes(b),'bytes':len(b)})
 rz.close();return acquired
def run_tracking(paths,conf):
 from ultralytics import YOLO
 from huggingface_hub import hf_hub_download
 weights=hf_hub_download(repo_id=MODEL_REPO,filename=MODEL_FILE);model=YOLO(weights);out={}
 # persist=True maintains ByteTrack state across sequential calls.
 for fr,p in paths:
  ...
  out[fr]=rows
 return out,weights
def detection_f1_for_conf(paths,gt_by_frame,conf):
 # Fresh detector, no tracker needed for threshold selection on the first 20 frames.
 from ultralytics import YOLO
 from huggingface_hub import hf_hub_download
 weights=hf_hub_download(repo_id=MODEL_REPO,filename=MODEL_FILE);model=YOLO(weights);tp=fp=fn=0
 for fr,p in paths:
 ...
 pr=tp/max(tp+fp,1);rc=tp/max(tp+fn,1);return 2*pr*rc/max(pr+rc,1e-9)
def contact_sheet(paths,gt,pred,outpath):
 from PIL import Image,ImageDraw,ImageFont
 chosen=paths[:8];thumbs=[]
 for fr,p in chosen:
  im=Image.open(p).convert('RGB');im.thumbnail((480,270));sx=im.width/1920 if im.width else 1;sy=im.height/1080 if im.height else 1;d=ImageDraw.Draw(im)
  # scale using original dimensions stored by annotations would be ideal; derive from actual source before thumbnail.
  orig=Image.open(p);ow,oh=orig.size;orig.close();sx=im.width/ow;sy=im.height/oh
  for g in gt.get(fr,[]):
   x1,y1,x2,y2=g['box'];d.rectangle((x1*sx,y1*sy,x2*sx,y2*sy),outline='lime',width=2)
  for q in pred.get(fr,[]):
   x1,y1,x2,y2=q['box'];d.rectangle((x1*sx,y1*sy,x2*sx,y2*sy),outline='cyan',width=2);d.text((x1*sx,y1*sy),f"P{q.get('track_id')}",fill='cyan')
  d.text((8,8),f'frame {fr}',fill='white');thumbs.append(im)
...
def main():
 ...
 acquired=acquire_frames(video);pmap={a['image_id']:pathlib.Path(a['path']) for a in acquired};anns=defaultdict(list)
 image_ids={im['id'] for im in video}
 ...
 val=[(im['id'],pmap[im['id']]) for im in video[:VAL_FRAMES]];test=[(im['id'],pmap[im['id']]) for im in video[VAL_FRAMES:]]
 cand=[]
 for c in (.05,.10,.15,.20,.30):cand.append((detection_f1_for_conf(val,anns,c),c))
 cand.sort(key=lambda x:(-x[0],x[1]));conf=cand[0][1];pred,weights=run_tracking(test,conf);m=metrics(pred,anns,[x[0] for x in test]);technical=m['f1']>=.45 and m['idf1']>=.35
 sheet=OUT/'seadronessee_mot_contact_sheet.jpg';contact_sheet(test,anns,pred,sheet)
 pack={'schema':'assetgraph-evidence/seadronessee-mot-e2e-v1','request_id':'REQ-003','dataset':{'official_dataset':'SeaDronesSee MOT','official_license':'CC0-1.0','transport_mirror':'ObjEarth Hugging Face','mirror_license_metadata':'requires review; underlying official source provenance retained','annotation_url':ANN_URL,'remote_zip_url':ZIP_URL,'annotation_sha256':sha_file(annp),'video_id':VIDEO_ID,'source_video':video[0].get('source',{}).get('video'),'frames_acquired':len(acquired),'frame_assets':acquired},'ground_truth':{'persistent_id_field':'track_id','categories':cats,'ignored_category_ids':sorted(ignored)},'model':{'repo':MODEL_REPO,'file':MODEL_FILE,'license':'AGPL-3.0','weights_sha256':sha_file(weights),'product_candidate':False},'selection':{'validation_frames':VAL_FRAMES,'conf_candidates':cand,'selected_conf':conf},'blind_tracking_test':{'frames':len(test),'metrics':m,'technical_component_pass':technical,'gate':{'detection_f1_gte':.45,'idf1_gte':.35}},'contact_sheet':str(sheet),'provenance_note':'RGB bytes, native track_id GT, source video mapping and telemetry originate from the same SeaDronesSee MOT release; model is benchmark-only because of AGPL.'}
 p=OUT/'seadronessee_mot_e2e.json';p.write_text(json.dumps(pack,indent=2));print(json.dumps({'pass':technical,'selected_conf':conf,'validation_detection_f1':cand[0][0],'metrics':m,'frames':len(test),'contact_sheet':str(sheet)},indent=2))
